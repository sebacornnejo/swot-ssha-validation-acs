...
% Two stacked panels (subplot layout).
% =========================================================================
...
% Apply the mask to the whole time series
for t = 1:ntime
...
% Pair the points (18 segments)
x_pairs = reshape(x_coords, 2, [])';
y_pairs = reshape(y_coords, 2, [])';
% Meridional phase velocity
R_earth = 6371000.0;
...
% -------------------------------------------------------------------------
% Per latitude (each row of HOV_disp) the dominant period inside the
% [2,10]-day band is estimated with a Lomb-Scargle periodogram (handles the
% NaN gaps of the SWOT revisits without interpolation). The per-latitude
% period distribution is right-skewed (period is 1/f), so the median is the
% robust estimator annotated on the Hovmoller.
% =========================================================================
band_lo_days = 2;  band_hi_days = 10;
f_band = linspace(1/band_hi_days, 1/band_lo_days, 400);   % cycles/day -> periods 10..2 d
...
T_iqr    = T_q3 - T_q1;
T_from_fmean = 1 / mean(1 ./ valid_P);                 % period of the mean frequency
...
% Medians per peak rank (1..5), computed per latitude: peaks are ranked by
% power at each latitude and the median of each rank is taken across
% latitudes. The rank-1 median equals T_median (the annotated value).
...
T_char_display = T_median;   % value annotated on the Hovmoller (panel a)

% =========================================================================
% 3c. Spectral analysis on unfiltered data (console only)
% -------------------------------------------------------------------------
% Rebuilds an unfiltered Hovmoller (masked, but not temporally filtered)
% and runs the same per-latitude Lomb-Scargle analysis, reporting the top-5
% peaks per latitude over the full resolvable spectrum and over the
% 2-10 day band.
% =========================================================================
...
f_full   = linspace(1/T_record, 0.5, 800);             % full resolvable band

% Single spectral pass over f_full per latitude: returns the top-5 peaks
% and the energy fractions used in section 3d.
char_rel = 0.15;   % +-15% window in period around the characteristic period
...
% Peaks restricted to the 2-10 d band (separate pass over f_band)
...
% -------------------------------------------------------------------------
% Band-limited variance fraction: per latitude, the Lomb-Scargle power is
% integrated over the 2-10 day band and divided by the integral over the
% full resolvable spectrum. Also reports the fraction concentrated in a
% narrow +-15% window around the characteristic period. Medians across
% latitudes.
% =========================================================================
...
% --- Figure canvas ---
fig = figure('Units','inches','Position',[1 1 11 13.5],'Visible','off');
%% PANEL 1: IB Hovmoller with phase-velocity lines
ax1 = subplot(2,1,1);
[T1, Y1] = meshgrid(regular_time_vec, common_lat);
pcolor(ax1, T1, Y1, HOV_disp); shading(ax1,'interp');
hold(ax1,'on');
% Zero contour
contour(ax1, T1, Y1, HOV_disp, [0 0], 'k:', 'LineWidth', 0.8);
% Digitized phase-velocity segments
for i = 1:size(x_pairs, 1)
    plot(ax1, [x_pairs(i,1), x_pairs(i,2)], [y_pairs(i,1), y_pairs(i,2)], ...
        '-o', 'LineWidth', 2.5, 'MarkerSize', 5, 'MarkerFaceColor',[1 1 1],'Color',[1 1 1]);
    plot(ax1, [x_pairs(i,1), x_pairs(i,2)], [y_pairs(i,1), y_pairs(i,2)], ...
         '-o', 'LineWidth', 1.5, 'MarkerSize', 4, 'MarkerFaceColor',[26, 224, 217]/255,'Color',[26, 224, 217]/255);
end

% Compact annotation of the characteristic period (white box), placed well
% inside the panel so it never overlaps the title or the ticks
x_txt = regular_time_vec(1) + 0.02*(regular_time_vec(end) - regular_time_vec(1));
y_txt = -42.3;
txt_str = sprintf('T \\approx %.1fd', T_char_display);
text(ax1, x_txt + 5.5, y_txt + 0.9, txt_str, ...
    'FontName','Times New Roman','FontSize',18, ...
    'HorizontalAlignment','left','VerticalAlignment','top', ...
    'BackgroundColor','w','EdgeColor','k','LineWidth',0.5,'Margin',1);

hold(ax1,'off');
title(ax1, ['(a)'], 'FontName','Times New Roman','FontSize',20);
ylabel(ax1, ['Latitude (' char(176) 'S)'], 'FontName','Times New Roman','FontSize',20);
xlim(ax1, [regular_time_vec(1) regular_time_vec(end)]);
ylim(ax1, [-53 -41.15]);
caxis(ax1, color_limits_sla); colormap(ax1, cmap);
h1 = colorbar(ax1); set(h1,'FontName','Times New Roman','FontSize',20);
ylabel(h1,'SSHA [m]', 'FontName','Times New Roman','FontSize',20);
datetick(ax1,'x','dd/mm','keepticks'); xtickangle(ax1,45);
set_axes_style(ax1);
%% PANEL 2: phase-velocity bars aligned in time
ax2 = subplot(2,1,2);
...
% 2. Mean velocity line
mean_h = yline(ax2, mean_v, '--', 'LineWidth', 2, 'Color', [0 0 0]);
% 3. Velocity bars
bar_h = bar(ax2, mean_x, v_phase, 1.5, 'FaceColor', [26, 224, 217]/255, 'EdgeColor', 'k');
bar_h.FaceAlpha = 0.85;
hold(ax2, 'off');
title(ax2, ['(b)'], 'FontName','Times New Roman','FontSize',20);
ylabel(ax2, 'Phase Velocity [m/s]', 'FontName','Times New Roman','FontSize',20);
xlim(ax2, [regular_time_vec(1) regular_time_vec(end)]);
ylim(ax2, [0 25]);
% Same x-axis format as the upper panel for alignment
datetick(ax2,'x','dd/mm','keepticks'); xtickangle(ax2,45);
set_axes_style(ax2);
...
% Match panel widths (the colorbar shrinks panel a)
drawnow;
...
function [P_per_lat, Pxx_mean] = band_periods_by_latitude(HOV, tvec, f)
    % For each latitude (row of HOV), returns the dominant period on the
    % frequency grid f (cycles/day) by Lomb-Scargle, plus the
    % latitude-averaged spectrum (row-normalized) as a robust check of the
    % common characteristic period.
    nlat      = size(HOV, 1);
    ...
    for r = 1:nlat
        ...
        if all(~isnan(Pxx)) && max(Pxx) > 0
            Pxx_acc = Pxx_acc + Pxx / max(Pxx);   % normalize each row's spectrum
            cnt     = cnt + 1;
        end
    end
...
function [P_peak, Pxx] = ls_dominant_period(t, x, f)
    % Manual Lomb-Scargle periodogram (no extra toolbox required).
    % t: times [days], x: values (NaN allowed), f: frequencies [cycles/day].
    % Returns the dominant period P_peak [days] and the spectrum Pxx over f.
    good = ~isnan(x) & ~isnan(t);
    t = t(good); x = x(good);
    if numel(x) < 8
        P_peak = NaN; Pxx = nan(numel(f), 1); return;
    end
    t = t(:) - t(1);          % center time (numerical stability)
...
function [Pmat, frac_band, frac_char] = per_latitude_spectral_diagnostics( ...
    ...
    % Single Lomb-Scargle pass over f_full per latitude, returning:
    %   - Pmat      : periods of the top-nPeaks peaks per latitude [nlat x nPeaks]
    %   - frac_band : energy (variance) fraction in the band [fb_lo,fb_hi]
    %                 relative to the resolvable total, per latitude
    %   - frac_char : energy fraction in a +-rel window (in period) around
    %                 the characteristic period Tc, per latitude
    nlat = size(HOV, 1);
...
function Pmat = per_latitude_peak_periods(HOV, tvec, f, nPeaks)
    % For each latitude (row of HOV): Lomb-Scargle periodogram, local maxima
    % (principal + secondary peaks) ranked by power, and the periods [days]
    % of the first nPeaks in a [nlat x nPeaks] matrix. Missing ranks = NaN.
    nlat = size(HOV, 1);
    Pmat = nan(nlat, nPeaks);
    for r = 1:nlat
        x = HOV(r, :);
        ...
        [~, order] = sort(Pxx(locs), 'descend');   % rank by power
        locs = locs(order);
        nTake = min(nPeaks, numel(locs));
...
function locs = local_maxima_indices(Pxx)
    % Indices of local maxima of Pxx, including the endpoints when they
    % exceed their single neighbour (so the global maximum is always
    % captured, even at a band edge). No findpeaks dependency.
    Pxx = Pxx(:);
    n = numel(Pxx);
    locs = [];
    for i = 1:n
...
function print_peak_rank_table(Pmat, label)
    % Prints, per peak rank (column of Pmat), the median period and its IQR
    % across latitudes, with the number of contributing latitudes.
    fprintf('  -- %s --\n', label);
    nP = size(Pmat, 2);
    for k = 1:nP

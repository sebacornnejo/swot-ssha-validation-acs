%% SWOT vs in situ SSHA validation statistics (input to Fig05)
% Evaluates SWOT L3 and in situ sea surface height anomalies (SSHA), both
% previously tide-corrected with reconstructed FES2022b tides, for each
% Dynamic Atmospheric Correction (DAC). Anomalies are centred over the
% common overlap; the script computes r, RMSE and R^2 per DAC (the R^2 that
% feeds the Fig05 bubble map) and saves per-station SSHA comparison series
% plus a statistics summary table.
...
for i_file = 1:length(input_files)
    ...
    % Bounding temporal limits per station
    if i_file == 2
    ...
    for i_dac = 1:length(dac_versions)
        ...
        % 2.1 Discard the initial transient common observation
        ...
        % 2.2 SLA centering over the common period
    ...
    % 5.5 Uniform axes limits and date ticks
